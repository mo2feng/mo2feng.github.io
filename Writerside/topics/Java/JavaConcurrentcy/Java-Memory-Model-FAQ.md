...
> [英文原文地址](http://www.cs.umd.edu/~pugh/java/memoryModel/jsr-133-faq.html)

> 使用deepl 机翻，略有修改

## 什么是内存模型(What is a memory model, anyway?)
...
## 其他编程语言（比如C++）又内存模型吗？（Do other languages, like C++, have a memory model?）

大多数其他编程语言（如 C 和 C++）在设计时并不直接支持多线程。这些语言针对在编译器和处理器平台架构的重排序行为的保护机制，在很大程度上取决于所使用的线程库（如 pthreads）、所使用的编译器以及运行代码的平台所提供的保障。

## JSR-133 是什么？（What is JSR 133 about?）

自 1997 年以来，人们不断的在《Java 语言规范》第 17 章中定义的 Java 内存模型中发现了一些严重缺陷。这些缺陷导致了一些令人困惑的行为（如观察到final字段的值被改变了）以及破坏了了编译器执行常见优化的能力。

Java 内存模型是一项雄心勃勃的工作，这是编程语言规范首次尝试合并一个能够在各种处理器架构中为并发提供一致语义的内存模型。不过，定义一个既一致又直观的内存模型比想象中要困难得多。JSR-133 为 Java 语言定义了新的内存模型，修正了早期内存模型的缺陷。为了实现JSR133，final和volatile的语义需要重新定义。

完整的语义请访问 [http://www.cs.umd.edu/users/pugh/java/memoryModel](http://www.cs.umd.edu/users/pugh/java/memoryModel)，但正式的语义并不适合胆小的人。令人感到惊讶和震惊的是发现同步等看似简单的概念其实是如此复杂。幸运的是，您无需了解正式语义的细节--JSR-133 的目标是创建一组正式语义，为 `volatile`、`synchronzied`和`final`如何工作提供一个直观的框架。
...
## 重排序是什么？(What is meant by reordering?)
...
## 旧内存模型有什么问题？(What was wrong with the old memory model?)

旧的内存模型中有几个严重的问题。这些问题很难理解，因此被广泛的违背。例如，在许多情况下，不允许JVM发生各种重排序行为。旧的内存模型中让人产生困惑的因素造就了JSR-133规范的诞生。
...
## 没有正确同步的含义是什么？(What do you mean by “incorrectly synchronized”?)
...
## 同步会做些什么呢？(What does synchronization do?)
...
对`volatile`字段的写操作happens before后续对相同volatile字段的读取操作。
...
> **重点注意**
>
> 对两个线程来说，为了正确建立happens before关系而在相同监视器上面进行同步是非常重要的。以下观点是错误的：当线程A在对象X上面同步的时候，所有东西对线程A可见，线程B在对象Y上面进行同步的时候，所有东西对线程B也是可见的。释放监视器和获取监视器必须匹配（也就是说要在相同的监视器上面完成这两个操作），否则，代码就会存在“数据竞争”。
>
> {style="note"}
## final字段如何改变它们的值？(How can final fields appear to change their values?)

我们可以通过分析String类的实现具体细节来展示一个final变量是如何可以改变的。

String对象包含了三个字段：一个character数组，一个数组的offset和一个length。实现String类的基本原理为：它不仅仅拥有character数组，而且为了避免多余的对象分配和拷贝，多个String和StringBuffer对象都会共享相同的character数组。因此，`String.substring()`方法能够通过改变`length`和`offset`，而共享原始的character数组来创建一个新的String。对一个String来说，这些字段都是final型的字段。
...
## 在新的JMM下final字段是如何工作的？(How do final fields work under the new JMM?)
...
## volatile是干什么用的？(What does volatile do?)
...
> **重点注意**
> 
> 对两个线程来说，为了正确的设置happens-before关系，访问相同的volatile变量是很重要的。以下的结论是不正确的：当线程A写volatile字段f的时候，线程A可见的所有东西，在线程B读取volatile的字段g之后，变得对线程B可见了。释放操作和获取操作必须匹配（也就是在同一个volatile字段上面完成）。
> 
>{style="note"}

## 新内存模型是否修复了双重锁检查问题？(Does the new memory model fix the "double-checked locking" problem?)
...
## 如果我要实现一个JVM，我需要做什么？（What if I'm writing a VM?）

You should look at [http://gee.cs.oswego.edu/dl/jmm/cookbook.html](http://gee.cs.oswego.edu/dl/jmm/cookbook.html) .

## 为什么要关注JMM？
为什么要关心？并发错误很难调试。它们通常不会在测试中出现，而是等到程序在重负载下运行时才出现，而且很难重现和捕获。

您最好提前花费额外的精力来确保您的程序正确同步；虽然这并不容易，但比起调试一个同步性很差的应用程序要容易得多。